...
const AddToGroupDialog = dynamic(
  () => import('@/components/user-details/add-to-group-dialog')
)
...
function UserDetailsPage() {
  ...
  const {
    ...
    () => (username ? `/api/users/${decodeURIComponent(username)}` : null),
    fetcher
  )
  const { data: groups = [], mutate: mutateGroups } = useSwr<GroupListType>(
    username ? `/api/users/${decodeURIComponent(username)}/groups` : null,
    fetcher
  )
  ...
  async function onResetUserPassword() {
    ...
    hideDialog()
    addNotification({
      message: 'Password reset successfully required for user',
      type: 'success',
    })
  }
